...
class DatabaseManager:
    ...
    def __init__(self):
        ...
        self._ocr_metrics_columns_ready = False
    ...
    def ensure_ocr_metrics_columns(self) -> None:
        """Добавляет технические OCR-колонки в analysis_results, если их нет."""
        if self._ocr_metrics_columns_ready:
            return

        cursor = self._get_cursor()
        try:
            if self.db_type == 'postgresql':
                cursor.execute("ALTER TABLE analysis_results ADD COLUMN IF NOT EXISTS ocr_engine VARCHAR(50)")
                cursor.execute("ALTER TABLE analysis_results ADD COLUMN IF NOT EXISTS ocr_duration_ms INTEGER")
                cursor.execute("ALTER TABLE analysis_results ADD COLUMN IF NOT EXISTS ocr_metrics JSONB")
            else:
                existing_columns = {
                    row[1] for row in cursor.execute("PRAGMA table_info(analysis_results)").fetchall()
                }
            ...
            self.conn.commit()
            self._ocr_metrics_columns_ready = True
            logger.debug("OCR metrics columns готовы")
        except Exception as e:
            logger.warning(f"Не удалось добавить OCR metrics columns: {e}")
            try:
                self.conn.rollback()
            except Exception:
                pass
    ...
    def get_pending_screenshots(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Ошибка получения pending скриншотов: {e}")
            try:
                self.conn.rollback()
            except:
                pass
    ...
    def save_analysis_result(
        ...
        ocr_metrics: Optional[Dict] = None,
    ):
        """
        ...
        Args:
            ...
            ocr_engine: OCR-движок, использованный для распознавания
            ocr_duration_ms: Время OCR-распознавания в миллисекундах
            ocr_metrics: JSON-метрики качества OCR
        """
        try:
            self.ensure_ocr_metrics_columns()
            cursor = self._get_cursor()
            metrics_json = json.dumps(ocr_metrics or {}, ensure_ascii=False, default=str)
            ...
            if self.db_type == 'postgresql':
                if self._ocr_metrics_columns_ready:
                    cursor.execute("""
                        INSERT INTO analysis_results (
                            ...
                            ocr_engine, ocr_duration_ms, ocr_metrics, analyzed_at
                        )
                        ...
                        ON CONFLICT (screenshot_id) DO UPDATE SET
                            ...
                            ocr_duration_ms = EXCLUDED.ocr_duration_ms,
                            ocr_metrics = EXCLUDED.ocr_metrics,
                ...
                # Для SQLite нужно добавить колонку details при необходимости
                try:
                    if self._ocr_metrics_columns_ready:
                        cursor.execute("""
                            INSERT INTO analysis_results (
                        ...
                        cursor.execute("""
                            INSERT INTO analysis_results (screenshot_id, ocr_text, category, confidence, analyzed_at)
                            VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
                            ON CONFLICT (screenshot_id) DO UPDATE SET
                                ocr_text = excluded.ocr_text,
                                category = excluded.category,
                    ...
                    logger.warning(f"Ошибка SQLite: {e}")
    ...
    def get_keywords(self, category: str) -> List[str]:
        ...
        Args:
            category: Категория ('work', 'user', 'productive', 'unproductive' для совместимости)
        ...
        # Для совместимости преобразуем старые категории
    ...
    def add_keyword(self, word: str, category: str, created_by: str = 'admin'):
        ...
        Args:
            word: Ключевое слово
            category: Категория ('productive' или 'unproductive')
    ...
    def load_default_keywords(self):
        ...
        # Ключевые слова для категории 'work' (рабочие)
        work_keywords = [
        ...
        # Ключевые слова для категории 'user' (личные)
        user_keywords = [
        ...
        try:
            ...
            cursor.execute("DELETE FROM keywords WHERE category IN ('productive', 'unproductive')")
            
            # Формируем批量 insert
            all_keywords = [(w, 'work') for w in work_keywords] + [(w, 'user') for w in user_keywords]
            
            if self.db_type == 'postgresql':
                from psycopg2.extras import execute_values
                execute_values(
                    cursor,
                    "INSERT INTO keywords (word, category, created_by) VALUES %s ON CONFLICT (word) DO NOTHING",
                    [(w, c, 'system') for w, c in all_keywords]
                )
            else:
                for word, cat in all_keywords:
                    cursor.execute(
                        "INSERT OR IGNORE INTO keywords (word, category, created_by) VALUES (?, ?, ?)",
                        (word, cat, 'system')
                    )
            
            self.conn.commit()
            logger.info(f"Загружено {len(all_keywords)} стандартных ключевых слов (work/user)")
    ...
    def get_productivity_stats(self, date_from: str, date_to: str) -> Dict:
        ...
        cursor.execute("""
            SELECT 
                ...
                SUM(CASE WHEN ar.category = 'work' THEN 1 ELSE 0 END) as work_count,
                SUM(CASE WHEN ar.category = 'user' THEN 1 ELSE 0 END) as user_count,
                SUM(CASE WHEN ar.category = 'unknown' THEN 1 ELSE 0 END) as unknown_count
            FROM screenshots s
    ...
    def get_daily_productivity(self, date_from: str, date_to: str) -> List[Dict]:
        ...
        cursor.execute("""
            SELECT 
                ...
                COUNT(s.id) as total,
                SUM(CASE WHEN ar.category = 'work' THEN 1 ELSE 0 END) as work,
                ROUND(
                    CAST(SUM(CASE WHEN ar.category = 'work' THEN 1 ELSE 0 END) AS NUMERIC) * 100 / 
                    NULLIF(COUNT(s.id), 0), 
                    2
